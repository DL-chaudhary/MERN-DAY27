...
function Login() {
  ...
  const handleChange=(e)=>{
    // let kname=e.target.name //kun ma type greko
    // let kvalue=e.target.value //ke type greko
  ...
  const submitData=async (e)=>{
   ...
   const resp=await axios.post("http://localhost:3000/read/login",hame)
  console.log("submit vyo hai")
  }
  ...
  <div className="bg-[#9f249f] rounded-2xl flex max-w-3xl p-5 items-center">
    <div className="md:w-1/2 px-8">
      <h2 className="font-bold text-3xl text-[#002D74]">Login</h2>
      <p className="text-sm mt-4 text-[#002D74]">If you already a member, easily log in now.</p>
      <form  onSubmit={submitData} action className="flex flex-col gap-4">

        
          <input onChange={handleChange} className="p-2 rounded-xl border w-full" type="text" name="name" id="text" placeholder="Name" />
                  <input onChange={handleChange} className="p-2 rounded-xl border mt-5 w-full" type="text" name="lastName" id="text" placeholder="LastName" />
        <input onChange={handleChange} className="p-2 mt-8 rounded-xl border" type="email" name="email" placeholder="email" />
        ...
        <button className="bg-[#002D74] text-white py-2 rounded-xl hover:scale-105 duration-300 hover:bg-[#206ab1] font-medium" type="submit">Submit</button>
      </form>
      ...
      <div className="mt-4 text-sm flex justify-between items-center container-mr">
        ...
        <button className="hover:border register text-white bg-[#002D74] hover:border-gray-400 rounded-xl py-2 px-5 hover:scale-110 hover:bg-[#002c7424] font-semibold duration-300"type="button">register</button>
      </div>
    </div>
